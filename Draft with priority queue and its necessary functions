...
public:
    ...
    Task(Task &tt){
        this->description = tt.getDes();
        this->name = tt.getName();
        this->priority = tt.getPri();
    }
...
public:
    ...
    void shiftUp(int position){
        ...
        while((position>=0)&&((tasks[parent(position)].getPri())<(tasks[(position)].getPri()))){
    ...
    void shiftDown(int position){
        ...
        if((l<numberOfTasks)&&((tasks[l].getPri())>(tasks[maxIndex].getPri())))
            maxIndex = l;
        
        if((r<numberOfTasks)&&((tasks[r].getPri())>(tasks[maxIndex].getPri())))
            maxIndex = r;
