...
public:
 ...
 void displayDeadlineReminders() const {
    cout << "Deadline Reminders:" << endl;
    auto currentTime = system_clock::to_time_t(system_clock::now());
    priority_queue<Task> pqCopy = pq;
    bool approachingDeadlines = false; // Flag to check if any deadlines are approaching within 1 hour
    while (!pqCopy.empty()) {
        Task task = pqCopy.top();
        pqCopy.pop();
        if (task.status == "Done" || task.status == "Late Done") {
            continue; // Skip tasks marked as done
        }
        time_t deadlineTime = parseDeadline(task.deadline);
        time_t timeDifference = deadlineTime - currentTime;
        if (timeDifference > 0 && timeDifference <= 3600) { // Deadline is within 1 hour
            approachingDeadlines = true;
            cout << "Task: " << task.name << " - Deadline is approaching within 1 hour!" << endl;
        }
    }

    if (!approachingDeadlines) {
        cout << "No deadlines approaching in 1 hour." << endl;
    }
}
...
int main() {
    ...
    while (true) {
        ...
        cout << "Press 5 to Display Deadline Reminders" << endl; // Changed option
        ...
        switch (choice) {
            ...
            case 5:
                taskManager.displayDeadlineReminders(); // Changed option
                break;
